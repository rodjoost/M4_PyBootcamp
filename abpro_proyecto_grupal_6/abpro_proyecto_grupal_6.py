...
class Vendedor:
    ...
    def vender(self, cantidad_vendida , producto, cliente = Cliente ):
...
productos = {nombre_producto: Producto(**producto_data[nombre_producto]) 
             for producto_data in data["stock"] 
             for nombre_producto in producto_data}

cliente1 = Cliente(5,"pedro","js@gfsd.com")
cliente1.agregar_saldo(100000)
vendedor_1 = Vendedor(555, "Waton", "Loyola", "5")
vendedor_1.vender(5, productos["manzana"], cliente1)  # Se pasa la instancia de la clase Producto y Cliente
print(vendedor_1)
...
orden = Orden_de_compra(1, True, productos["manzana"])  # Se pasa la instancia de la clase Producto
print(orden.detalle())